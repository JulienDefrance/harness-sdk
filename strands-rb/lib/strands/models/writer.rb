...
module Strands
  module Models
    # Writer AI model provider implementation.
    #
    # Implements the streaming interface using the Writer Chat API
    # with Server-Sent Events (SSE) for streaming responses.
    #
    # @example Basic usage
    #   model = Strands::Models::Writer.new(
    ...
    class Writer
      ...
      # Default Writer API base URL
      DEFAULT_BASE_URL = "https://api.writer.com"

      # Context overflow error messages from Writer
      ...
      # Initialize the Writer model provider.
      #
      # @param model_id [String] the model identifier
      # @param api_key [String, nil] Writer API key (defaults to ENV["WRITER_API_KEY"])
      # @param base_url [String] API base URL
      # @param params [Hash] additional model parameters
      def initialize(model_id:, api_key: nil, base_url: DEFAULT_BASE_URL, **params)
        @config = {
          model_id: model_id,
          api_key: api_key || ENV.fetch("WRITER_API_KEY", nil),
          base_url: base_url,
          params: params
        }
      end
      ...
      # Stream a conversation with the Writer model.
      #
      ...
      def stream(messages, system_prompt: nil, tools: nil, tool_choice: nil, **kwargs)
        ...
        uri = URI.join(@config[:base_url], "/v1/chat")
      ...
      # Format the request body for the Writer Chat API.
      #
      # @param messages [Array<Hash>] conversation messages
      # @param system_prompt [String, nil] system prompt
      ...
      def map_finish_reason(finish_reason)
        ...
        when "content_filter" then :content_filtered
        else :end_turn
        end
      end
      ...
      def handle_error_response(response)
        ...
        when 400
          ...
          raise Strands::Error, "Writer API error (400): #{error_message}"
        else
          if CONTEXT_OVERFLOW_MESSAGES.any? { |msg| error_message.downcase.include?(msg) }
            raise Types::Exceptions::ContextWindowOverflowError, error_message
          end
          raise Strands::Error, "Writer API error (#{response.code}): #{error_message}"
        end
      end
    end
  end
