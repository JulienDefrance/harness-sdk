...
module Strands
  module Models
    # LiteLLM proxy model provider implementation.
    #
    # Connects to a LiteLLM proxy server that exposes an OpenAI-compatible API.
    # The model_id is passed through as-is to allow routing via the proxy
    # (e.g., "anthropic/claude-3-sonnet").
    #
    # @example Basic usage
    #   model = Strands::Models::LiteLLM.new(
    ...
    class LiteLLM
      ...
      # Initialize the LiteLLM model provider.
      #
      # @param model_id [String] the model identifier passed to the proxy
      # @param api_key [String, nil] API key for the LiteLLM proxy
      # @param base_url [String] URL of the LiteLLM proxy server
      ...
      # Stream a conversation via the LiteLLM proxy.
      #
      ...
      # Format the request body for the OpenAI-compatible endpoint.
      #
      # @param messages [Array<Hash>] conversation messages
      # @param system_prompt [String, nil] system prompt
      ...
      # @return [Hash] function tool definition
      ...
      def handle_error_response(response)
        ...
        when 400
          ...
          raise Strands::Error, "LiteLLM API error (400): #{error_message}"
        else
          if CONTEXT_OVERFLOW_MESSAGES.any? { |msg| error_message.include?(msg) }
            raise Types::Exceptions::ContextWindowOverflowError, error_message
          end
          raise Strands::Error, "LiteLLM API error (#{response.code}): #{error_message}"
        end
      end
    end
  end
